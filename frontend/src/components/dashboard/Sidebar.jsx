...
const Sidebar = ({ sidebarItems, activeTab, setActiveTab }) => {
  ...
  return (
    <div 
      className={`${isHovered ? 'w-64' : 'w-16'} bg-white shadow-xl fixed h-full z-10 transition-all duration-300 ease-in-out border-r border-gray-200`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center shadow-lg flex-shrink-0">
            <Brain className="h-5 w-5 text-white" />
          </div>
          <div className={`transition-all duration-300 ${isHovered ? 'opacity-100 w-auto' : 'opacity-0 w-0 overflow-hidden'}`}>
            <h1 className="text-lg font-bold text-gray-800 whitespace-nowrap">Neucom</h1>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 bg-white">
        ...
        >
          ...
          <div className={`transition-all duration-300 ${isHovered ? 'opacity-100 w-auto' : 'opacity-0 w-0 overflow-hidden'}`}>
            <p className="text-sm font-medium text-gray-800 whitespace-nowrap">Administrator</p>
